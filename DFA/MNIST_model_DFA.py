...
class DFA_Linear(nn.Linear):
    def __init__(self, in_features, out_features, bias = True, device = None, dtype = None):
        ...
        self.backward_weight = nn.Parameter(torch.Tensor(size = (in_features, out_features)), requires_grad = False) # B --> same shape as W transpose
        if self.bias is not None:
            self.backward_bias = nn.Parameter(torch.Tensor(size = (in_features, out_features)), requires_grad = False)
    ...
    def dfa_backward_hook(module, grad_input, grad_output):
        ...
        if grad_input[0] is None:
            ...
            grad_dfa = torch.matmul(module.global_loss_gradient, module.backward_weight.T) # B*e --> [in_features, B]
